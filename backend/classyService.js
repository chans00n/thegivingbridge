...
async function getClassyAppAccessToken() {
  ...
  if (!clientId || !clientSecret) {
    console.error(
      "Classy client ID or secret is not configured in environment variables.",
    );
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error(
        `Error fetching Classy token: ${response.status} ${errorBody}`,
      );
      throw new Error(
        `Failed to fetch Classy app access token: ${response.status} - ${errorBody}`,
      );
    }
...
async function makeClassyGetRequest(endpoint, queryParams = "") {
  ...
  try {
    ...
    if (!response.ok) {
      const errorBody = await response.text();
      console.error(
        `Classy API Error (${response.status}) for ${url}: ${errorBody}`,
      );
      throw new Error(
        `Classy API request failed for ${endpoint}: ${response.status} - ${errorBody}`,
      );
    }
...
async function getCampaignsByOrganization(orgId, queryParams = "") {
  if (!orgId) {
    ...
    if (!orgId) {
      console.error(
        "Classy Organization ID is not provided and not found in environment variables.",
      );
